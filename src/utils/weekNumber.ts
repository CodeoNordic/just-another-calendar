...
export default function weekNumber(dt: Date) 
{
    moment.locale('no', {
        week: {
            dow: typeof window._config?.firstDayOfWeek === 'string' ? Math.max(weekDays.indexOf(window._config?.firstDayOfWeek.toLowerCase().substring(0,3)), 1) : window._config?.firstDayOfWeek ?? 1
        }
    });
